...
export class GoDeliver extends Plan {
    ...
    async execute(_) {
        ...
        // Filter delivery tiles to those unoccupied and without a parcel already
        const freeDelivery = this.reachableDelivery.filter(tile => {
            const occupiedByAgent = Array.from(this.agent.agents.values())
                .some(a => a.x === tile.x && a.y === tile.y);
            return !occupiedByAgent;
        });
...
export class Patrolling extends Plan {
    ...
    async execute(_, easyPatrol) {
        ...
        if (easyPatrol) {
            if (this.agent.me.x === this.reachableSpawn[0].x && this.agent.me.y === this.reachableSpawn[0].y) {
                const x = this.agent.me.x;
                const y = this.agent.me.y;
                await this.agent.client.emitMove({x, y});
                this.agent.sensingEmitter.emit("agent_stay");
                return true;
            }
            return await this.subIntention(["go_to", this.reachableSpawn[0].x, this.reachableSpawn[0].y]);
        }
        ...
        return await this.subIntention(["go_to", target.x, target.y]);
    }
}
    ...
    async execute(_, tx, ty) {
        ...
        while (path.length) {
            ...
            if(!status){
                ...
                if (!path?.length) {
                    console.warn("Replan failed — giving up.");
                    throw new Error("Plan could not complete");
                }
            ...
            // Opportunistic pickup if we pass a parcel
            for (const p of this.agent.parcels.values()) {
...
export class IntentionRevision {
    constructor(agent) {
        ...
        this.MAX_DELIVERY_TRY = 3; // Max delivery attempts before giving up
    ...
    computeParcelUtility(from, parcel) {
        ...
        const parcels_decay = Math.ceil(myPath?.length * this.agent.MOVEMENT_DURATION() * 10 / this.agent.PARCEL_DECADING_INTERVAL());
        ...
        const racer = Array.from(this.agent.agents.values()).find(other => {
            if (other.id === this.agent.me.id || !other.last_move) {
                return false;
            }
    ...
    computeDeliveryUtility() {
        ...
        const pathHome = this.agent.searchFunction(this.agent.me, dest, this.agent.agents);
        //round p to closest bigger integer
        const parcels_decay = Math.ceil(pathHome?.length * this.agent.MOVEMENT_DURATION() * 3 / this.agent.PARCEL_DECADING_INTERVAL());
    ...
    async loop() {
        ...
        const reachableSpawn = [];
        const reachableDelivery = [];

        // Partition tiles by type: '1'=spawn, '2'=delivery
        for (const tile of this.agent.myIslandMap.tiles.values()) {
            if (tile.type == '1') reachableSpawn.push(tile);
            else if (tile.type == '2') reachableDelivery.push(tile);
        }

        // If only one spawn tile, mark for easy patrol
        if (reachableSpawn.length === 1) this.easyPatrol = true;
        ...
        while (true) {
            ...
            this.current = {
                type: job.type,
                intent: new Intention(this.agent, predicate, reachableSpawn, reachableDelivery)
            };
            ...
            try {
                ...
                // On failure, stop and replan
                // console.log("Intent failed, replanning:", job.type, e);
                this.current.intent.stop();
                this.current = null;
                this._replan();
            }
